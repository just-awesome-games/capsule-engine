...
| `src/asset-sources/sprites/player.sheet.json`      | The player's sheet: the six frames it cuts from `textures/player.png`, each with its bottom-centre pivot, and the looping `idle` and `walk` clips over them in ticks. The build compiles it into `GameSprites.Player`; nothing of it ships.                                                  |
| `src/asset-sources/textures/`                     | Texture sources: the player's frame strip, the sensor's field, and the terrain atlas the tileset cuts. Each becomes a `GameAssets.Textures.<Name>` handle, ships to `assets/textures/`, and is resident from boot.                                                                          |
