...
internal static class Program
{
    ...
    {
        int? tileSize = null;
        if (args is ["--tile-size", string declared, .. string[] rest])
        {
        ...
        return args switch
        {
            ["--maps-from", string list] =>
                MapTool.ImportTiledFromList(outputDirectory, list, tileSize, Console.Out, Console.Error),
            [_, ..] => MapTool.ImportTiled(outputDirectory, args, tileSize, Console.Out, Console.Error),
            _ => UsageError(),
        };
    }
