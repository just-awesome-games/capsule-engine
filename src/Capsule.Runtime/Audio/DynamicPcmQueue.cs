...
// format is fixed at construction, so a queue is reused only for a clip of the same rate and channel
// count.
internal sealed class DynamicPcmQueue : IPcmQueue
{
    ...
    // A mono source is rotated across the field. A stereo one rotates only where the device reports
    ...
    // Immediate, not at the end of what is queued. Dropping the buffers leaves the queue clean for the
    // next voice.
