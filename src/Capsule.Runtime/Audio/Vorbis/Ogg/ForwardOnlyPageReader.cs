...
namespace Capsule.Runtime.Audio.Vorbis.Ogg
{
    class ForwardOnlyPageReader : PageReaderBase
    {
        ...
        // Capsule: this reader queues pages ahead of the packet reader for an unbounded run
        // (_pageQueue in ForwardOnlyPacketProvider), so it opts out of PageReaderBase's pooled
        // ring and keeps one fresh buffer per page, exactly as upstream did.
        protected override byte[] RentPageBuffer(int size) => new byte[size];
