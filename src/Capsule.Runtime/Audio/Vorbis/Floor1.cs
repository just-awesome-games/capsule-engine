...
namespace Capsule.Runtime.Audio.Vorbis
{
    ...
    class Floor1 : IFloor
    {
        ...
        // Capsule: one Data per channel, reused every packet. Mapping.DecodePacket indexes floor
        // data by channel. A packet's floor data is fully consumed before the next packet's Unpack
        // call for that channel. Overwriting it in place is safe.
        Data[] _dataByChannel;

        // Capsule: Apply's scratch, reused every call instead of `new bool[64]`/`new int[64]` per
        // packet. Apply runs on the main decode thread. It is never reentrant for one Floor1.
        readonly bool[] _stepFlags = new bool[64];
        readonly int[] _finalY = new int[64];
        ...
        {
            // Capsule: sized once at setup, one Data per channel for the life of the floor.
            _dataByChannel = new Data[channels];
            for (int c = 0; c < channels; c++)
            {
                _dataByChannel[c] = new Data();
            }
        ...
        {
            // Capsule: reused per channel instead of `new Data()` per packet (see _dataByChannel).
            // Reset every field a fresh Data would have started from. A fresh Data's Posts array
            // is zero-filled. Posts[postCount] below is left unwritten whenever a subclass has no
            // book (book == null is a valid, ordinary case, not a bail), while postCount still
            // advances past it. A fresh array reads that gap as zero. This reused array must be
            // cleared to match, or it reads whatever a previous packet left there instead.
            var data = _dataByChannel[channel];
            data.PostCount = 0;
            data.ForceEnergy = false;
            data.ForceNoEnergy = false;
            Array.Clear(data.Posts, 0, data.Posts.Length);
        ...
        {
            // Capsule: instance scratch (see _stepFlags/_finalY) instead of `new bool[64]`/`new
            // int[64]` per Apply call. data.PostCount is always 0 or the full post count for this
            // floor's setup, so every index below it is written before Apply reads it back. A
            // stale entry beyond PostCount from an earlier, larger decode is never read.
            var stepFlags = _stepFlags;
            stepFlags[0] = true;
            stepFlags[1] = true;
