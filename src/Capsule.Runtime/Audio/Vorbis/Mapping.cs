...
namespace Capsule.Runtime.Audio.Vorbis
{
    class Mapping : IMapping
    {
        ...
        public void Init(IPacket packet, int channels, IFloor[] floors, IResidue[] residues, IMdct mdct)
        {
            ...
            // Capsule: sized once here instead of per packet in DecodePacket.
            _floorData = new IFloorData[_channelFloor.Length];
        ...
        public void DecodePacket(IPacket packet, int blockSize, int channels, float[][] buffer)
        {
            ...
            // Capsule: reused instance scratch (see _floorData/_noExecuteChannel) instead of
            // allocating fresh arrays every packet.
            var floorData = _floorData;
            var noExecuteChannel = _noExecuteChannel;
