...
namespace Capsule.Runtime.Audio.Vorbis
{
    ...
    class Residue0 : IResidue
    {
        ...
        // Capsule: growable instance scratch for Decode/WriteVectors, replacing `new int[_channels,
        // partitionWords][]` and `new int[steps]` per call. Decode is not reentrant for one
        // Residue0 instance, so one field per array is safe. partWordCache is flat (channel *
        // partitionWords + entry) instead of 2D, since a 2D array cannot grow one dimension in
        // place without discarding the whole array.
        int[][] _partWordCache;
        int[] _entryCache;
        ...
        virtual public void Decode(IPacket packet, bool[] doNotDecodeChannel, int blockSize, float[][] buffer)
        {
            ...
            if (n > 0 && Array.IndexOf(doNotDecodeChannel, false) != -1)
            {
                ...
                var neededWords = _channels * partitionWords;
                var partWordCache = _partWordCache;
                if (partWordCache == null || partWordCache.Length < neededWords)
                {
                    partWordCache = _partWordCache = new int[neededWords][];
                }
        ...
        virtual protected bool WriteVectors(ICodebook codebook, IPacket packet, float[][] residue, int channel, int offset, int partitionSize)
        {
            ...
            var entryCache = _entryCache;
            if (entryCache == null || entryCache.Length < steps)
            {
                entryCache = _entryCache = new int[steps];
            }
