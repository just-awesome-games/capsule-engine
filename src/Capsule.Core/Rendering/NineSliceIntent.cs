...
{
    /// <summary>
    /// The <see cref="Size"/> box at <see cref="Position"/>, and empty where it has no extent. A corner
    /// that overhangs a panel smaller than its insets draws past this box.
    /// </summary>
    // Negated comparisons reject a NaN extent along with the non-positive ones, as the expansion does.
    public Rect Bounds => !(Size.X > 0f) || !(Size.Y > 0f) ? default : new Rect(Position, Size);

    // The cut on both axes, written into the caller's three-slot spans. A slice with no texels or no extent
    // is left at zero extent and is not drawn.
    ...
    // One axis of the cut. The far edge is placed from the target's far side, so it stays on that edge
    // however much the middle was squeezed.
...
// One slice of one axis: where it is cut from in texels, and where it lands in the drawn space's units.
