...
/// <summary>
/// An axis-aligned box in world units. <paramref name="Min"/> is no greater than
/// <paramref name="Max"/> on either axis, and the game picks which way each axis points.
/// </summary>
...
public readonly record struct Aabb2D(Vector2 Min, Vector2 Max)
{
    ...
    /// <param name="corner">The lower corner on both axes, the top left in a Y-down world.</param>
    ...
    {
        Guard.Finite(size, nameof(size));

        if (size.X < 0f || size.Y < 0f)
        {
            throw new ArgumentOutOfRangeException(nameof(size), size, "Expected a non-negative extent on each axis.");
        }
    ...
    // Half the box's outline length, the 2D surface-area heuristic the dynamic tree balances by.
    ...
    /// <summary>Whether <paramref name="other"/> lies inside this box.</summary>
    ...
    // This box grown by margin world units on every side.
    ...
    // The smallest box holding this one before and after the translation.
    ...
    // Two bounds of the same sign can overflow a float when summed, so that case walks half the
    // width out from Min.
