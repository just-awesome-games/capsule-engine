...
`Capsule.Scenes` holds the world a game plays in: scenes, entities, components, cameras, tile maps, the screen-space layer an interface is drawn on, the `FocusNavigator` a menu on that layer is driven by, the scene document a scene is composed from, and the input driver and `SceneRun` a test drives it from.

Referenced by: game logic projects.

API starting points: `StepContext`, `InputState`, `DeviceSnapshot`, `RandomSource`, `AudioMixer` and `AudioClip` in Core; `CollisionWorld2D`, `Shape2D` and `CollisionFilter` in Collision; `Scene`, `Entity`, `Component`, `SceneSimulation`, `SceneRun`, `Camera`, `SpriteRenderer`, `Label`, `ColorRect`, `NineSlice`, `Anchor`, `AudioSource`, `Collider2D` and `IInputDriver` in Scenes.
