...
API starting points: `StepContext`, `InputState`, `DeviceSnapshot`, `RandomSource`, `AudioMixer` and `AudioClip` in Core; `CollisionWorld2D`, `Shape2D` and `CollisionFilter` in Collision; `Run`, `Scene`, `Entity`, `ScreenEntity`, `Component`, `SceneSimulation`, `SimulationHost`, `Camera`, `SpriteRenderer`, `Label`, `ColorRect`, `NineSlice`, `Anchor`, `AudioSource`, `Collider2D` and `IInputDriver` in Scenes.
