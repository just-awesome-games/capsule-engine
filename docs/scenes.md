...
- `zIndex` is the entry's draw band, applied to the spawned entity's `ZIndex` after it is constructed. Absent is 0, which is what the writer emits for an unbanded entry. It is an ordering key, not a coordinate: higher draws later, and entries sharing a band draw in file order. Both entry types carry it, and a tile map's is the band every one of its tiles draws in.
