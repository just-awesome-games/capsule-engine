...
- Inside `Capsule.Runtime`, operating-system, file-system, window and MonoGame-platform assumptions stay in the desktop files — `SdlPlatform.cs`, `WindowsForeground.cs`, `CapsuleGame.cs`, `CrashLog.cs`, `ConsoleLogSink.cs`, `Persistence/DirectorySaveStorage.cs`, `Persistence/LocalFolder.cs`, `Assets/`, `Audio/`, `Input/`, `Rendering/FrameRenderer.cs`; the boot surface (`CapsuleBoot`, `CapsuleEngine`, `EngineBuilder`) carries no desktop concept beyond an application title and texture sampling.
