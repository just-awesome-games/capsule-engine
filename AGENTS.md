...
The development overlay under `src/Capsule.Runtime/DevTools/` and the guarded block in `CapsuleGame` are the whole of it: deleting them must leave the engine compiling and every other test passing, so a host seam the overlay needs is added as generic host machinery, never shaped for the overlay.

Place a type by the grid in `docs/architecture.md` § Placement: the assembly by what it depends on, the namespace and folder by its domain. A new domain adds a row there; a new assembly needs a reference direction the compiler must enforce. Foundational entry points may remain at an assembly root. Do not create speculative folders or new assemblies merely to reduce a directory's file count; `docs/project-layout.md` governs game layout, not engine layout.
