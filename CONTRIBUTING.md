...
Capsule is developed for JAG Studios' games in public. A change is accepted when it improves an existing engine capability or its documentation. A new subsystem, hook or option needs a consuming game's use case, so open an issue first. The rules a change is held to are in [`AGENTS.md`](AGENTS.md).
...
[`.githooks/pre-commit`](.githooks/pre-commit) gates every commit: a locked restore, the build, the format check, and the tests. CI adds pack, the package-mode sample, and NativeAOT publishes of the sample shell and `tests/Capsule.AotSmoke`, whose binary it then runs. The smoke asserts both directions of the `CapsuleShipping` axis, so CI runs it twice: once from an ordinary build, and once published. Releases follow [`RELEASING.md`](RELEASING.md).
