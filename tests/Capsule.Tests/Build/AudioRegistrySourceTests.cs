...
public sealed class AudioRegistrySourceTests
{
    ...
    public void TheRenderedFile_IsTheSameWhateverOrderTheSourcesArriveIn()
    {
        ...
        Assert.Equal(
            Rendered([stone, theme]),
            Rendered([theme, stone]));
    }

    private static string Rendered(IReadOnlyList<AudioSourceClip> clips)
    {
        string? generated = AudioRegistrySource.Render(clips, out _, out string? because);
        Assert.True(generated is not null, because);
