...
public sealed class ShapeTests
{
    ...
    {
        Assert.Throws<ArgumentException>(() => Shape.Box(new Aabb(new Vector2(-3e38f, -1f), new Vector2(3e38f, 1f))));
        Assert.Throws<ArgumentException>(() => Shape.Box(Vector2.Zero, new Vector2(3e38f, 3e38f)));
        Assert.Throws<ArgumentException>(() => Shape.Circle(Vector2.Zero, 2e38f));
    }
    ...
    {
        Assert.Throws<ArgumentException>(() => Shape.Polygon(
            [new Vector2(-3e38f, -1f), new Vector2(3e38f, -1f), new Vector2(0f, 1f)]));

        Assert.Throws<ArgumentException>(() => Shape.Capsule(new Vector2(-3e38f, 0f), new Vector2(3e38f, 0f), 1f));
    }
    ...
    {
        Shape unit = Shape.Box(Vector2.Zero, new Vector2(1f, 1f));
        ...
        Shape capsule = Shape.Capsule(new Vector2(-10f, 0f), new Vector2(10f, 0f), 1f);
    ...
    public void AShapeAtTheFurthestCoordinateItKeepsItsExtent_IsPlacedAndFound()
    {
        ...
        Shape placed = unit.Translated(new Vector2(Furthest, 0f));
    ...
    public void AShapeAtTheLargestExtentThatStaysFinite_IsBuiltAndAnsweredFinitely()
    {
        ...
        Assert.True(world.Raycast(new Vector2(-10f, 5f), Vector2.UnitX, 100f, CollisionFilter.Everything, out RayHit hit));
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => Shape.Circle(Vector2.Zero, 0f));
        Assert.Throws<ArgumentOutOfRangeException>(() => Shape.Circle(Vector2.Zero, float.NaN));
    }
    ...
    public void Capsule_RejectsCoincidentEndpoints()
    {
        ArgumentException error = Assert.Throws<ArgumentException>(
            () => Shape.Capsule(new Vector2(4f, 4f), new Vector2(4f, 4f), 2f));
    ...
    {
        Assert.Throws<ArgumentException>(() => Shape.Box(new Aabb(new Vector2(4f, 0f), new Vector2(0f, 8f))));
        Assert.Throws<ArgumentException>(() => Shape.Box(new Aabb(Vector2.Zero, new Vector2(8f, 0f))));
    }

    [Fact]
    public void Polygon_RejectsAPointSetOutsideTheUnionsLimits()
    {
        Assert.Throws<ArgumentException>(() => Shape.Polygon([Vector2.Zero, Vector2.One]));
        Assert.Throws<ArgumentException>(() => Shape.Polygon(
            [
    ...
    {
        ArgumentException concave = Assert.Throws<ArgumentException>(() => Shape.Polygon(
    ...
    {
        ArgumentException error = Assert.Throws<ArgumentException>(() => Shape.Polygon(
    ...
    public void Polygon_AcceptsEitherWindingOrderAndStillFacesItsNormalsOutward()
    {
        ...
        {
            CollisionWorld world = new();
            world.Add(Shape.Polygon(points), Vector2.Zero, world.Tag("target"), CollisionFilter.None);

            Assert.True(world.Raycast(Vector2.Zero, Vector2.UnitX, 100f, CollisionFilter.Everything, out RayHit hit));
    ...
    {
        Shape polygon = Shape.Polygon([new(0f, 0f), new(8f, 0f), new(8f, 4f), new(0f, 4f)]);

        Assert.Equal(ShapeKind.Box, polygon.Kind);
        Assert.Equal(ShapeKind.Polygon, Shape.Polygon([new(0f, 0f), new(8f, 0f), new(8f, 4f), new(0f, 4f)], 1f).Kind);
    }

    [Fact]
    public void Bounds_CoverThePointsAndTheRadius()
    {
        Shape capsule = Shape.Capsule(new Vector2(4f, 4f), new Vector2(4f, 12f), 3f);
    ...
    {
        Shape moved = Shape.Box(Vector2.Zero, new Vector2(8f, 8f)).Translated(new Vector2(10f, 20f));
    ...
    {
        Shape circle = Shape.Circle(Vector2.Zero, 4f);
